...
// Config
const config = require('shapeshift-common-library')('shapeshift-exchange-management-api').get('bittrex')
...
const getTickers = function (symbols, callback) {
  let url = baseURL + 'getmarketsummaries'
  request.get({url: url}, function (err, response, body) {
    if (err) {
      callback(err)
    } else if (!body || !JSON.parse(body).result) {
      let newError = new Error('The Bittrex ticker update request did not return the correct data. The endpoint returned: ' + body)
      callback(newError)
    } else {
      let results = JSON.parse(body).result
      let formatted = {}
      results.map(function (ticker) {
        formatted[ticker.MarketName] = ticker
      })
